...
const SecondaryButton = ({className, children, onClick, disabled, title, style, onMouseEnter, onMouseLeave}) => {
  return (
    ...
    <button onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={onClick} className={`border rounded-md px-6 h-12 ${className}`} disabled={disabled} title={title} type="button" style={style}>{children}</button>
